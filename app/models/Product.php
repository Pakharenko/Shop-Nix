...
class Product extends Model
{
    ...
    {
        return $this->findBySql("SELECT * FROM products WHERE is_new = 1 ORDER BY id DESC LIMIT " . self::LIMIT_PAGE);
    }

    public function getHitsProducts()
    {
        return $this->findBySql("SELECT * FROM products WHERE is_hits = 1 ORDER BY id DESC LIMIT " . self::LIMIT_PAGE);
    }

    public function getPopularProducts()
    {
        return $this->findBySql("SELECT * FROM products WHERE is_popular = 1 ORDER BY id DESC LIMIT " . self::VIEW_PAGE_LIST);
    }
    ...
    {
        return $this->findBySql("SELECT * FROM products WHERE category_id = $categoryId ORDER BY id DESC LIMIT " . self::VIEW_PAGE_LIST);
    }
